...
const addUserExercise = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (existingUserExercise) {
      return res.status(400).json({ error: `Exercise '${name}' already exists.` });
    }
...
const updateUserExercise = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (duplicateExercise) {
      return res.status(400).json({ error: `Exercise '${newName}' already exists.` });
    }
...
const deleteUserExercise = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
